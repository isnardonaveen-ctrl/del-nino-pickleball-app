...
def show_home_page():
    ...
    with col1:
        ...
        **Ready to hit the court?** Come join the movement and see why we’re the fastest-growing community in the city!
        """)
    ...
    with col2:
        ...
        st.image("qr-code.png", caption="Scan to Register") # Use your actual file name
    ...
    if password == "Admin123":
        ...
        if raw_sheet_df is not None:
            ...
            for col in target_cols:
                if col not in raw_sheet_df.columns: raw_sheet_df[col] = 0
            
            clean_df = pd.merge(calendar_df, raw_sheet_df.dropna(subset=["Date"]), on="Date", how="left")
            for col in target_cols: clean_df[col] = pd.to_numeric(clean_df[col], errors='coerce').fillna(0)
            ...
            col4.metric("Bank Balance", f"₱{clean_df['Net Cash for Today'].sum():,.2f}")
